...
`verify_evidence_manifest.py`, `verify_evidence_history.py`,
`check_assurance_anchor.py`, `evidence_profile.py` and `finalize_collection.py`,
and it does not inherit their job. It does not decide whether a retained record
qualifies anything, does not maintain an anchor file or a retraction registry,
and has no notion of an active record.
...
It is not a translator of last resort. This repository's 11 retained envelopes
...
def derive(raw: bytes, derivation: dict[str, Any]) -> bytes:
    ...
    if operation == "replace":
        ...
        if find == replace:
            raise ViewError(
                "a replacement that writes back the value already present changes no byte "
                "and derives nothing"
            )
...
def uncommitted_evidence_changes() -> list[str]:
    ...
    the ones this migration removed.
...
def retained_envelopes() -> list[Path]:
    paths = sorted(EVIDENCE.glob("tl-rewrite-v01-*/evidence-envelope.json"))
    if not paths:
...
def retained_report(mapper: Callable[..., dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "statement": (
            ...
            "pass and not converted into a defect of these records. The number is measured "
            "here rather than inherited from a sibling: tl-syntax reported the identical "
            "refusal over 23 envelopes and tl-parse over 12, while quire-analyze's Markdown "
            "summaries read as 'unreadable' instead. Filed upstream as "
...
def census(mapper: Callable[..., dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schemaVersion": "tl-rewrite.legacy-compatibility-census/v1",
        "mapping_authority": (
            "engineering_assurance.verification_semantics.map_pgm01_bytes from the pinned "
...
def run_mutation_probes(mapper: Callable[..., dict[str, Any]]) -> dict[str, Any]:
    ...
    def erasing_record_id(raw: bytes, **kwargs: Any) -> dict[str, Any]:
        """Report every record under the same identity, so tampered and unreadable merge."""
        view = mapper(raw, **kwargs)
        view["source_record_id"] = "pgm-01-abcdef0"
        return view
