...
| FR-006-AC-7 | No repository-local generic runner, evidence envelope, manifest, identity framework, retention store, audit store, anchor verifier, retraction registry, evidence schema, retained-evidence directory, compatibility view, or aggregate verdict remains in the repository. The removal census enumerates tracked and untracked-not-ignored repository paths through Git, constrains its exact denials, tracked areas and declaration exemptions, scans raw bytes, fails closed with a named diagnostic when it cannot enumerate or read a path, and retains a preferred-Makefile control for the removed compatibility-view target. | Test (TC-029) |
