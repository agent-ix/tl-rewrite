...
- **2026-08-31** - Replaced transitive provenance-set unions with a budgeted linear reachability
  ...
  `9e736b25dcc1` after restoring the pinned validator packages. The new traversal deliberately
  charges each unique retained-source visit, so reported `workUnits` and tight budget outcomes can
  increase relative to the earlier uncharged implementation.
