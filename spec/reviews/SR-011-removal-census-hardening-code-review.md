...
| FND-1103 | high | FIXED. One twelve-name executable set covers the deleted scripts, targets, compatibility-view spellings and both schema filenames. It is cross-checked against the declaration's unsealed `census_controls` so the authorial and executable lists are separately reviewable; neither is claimed as independent authority. The production exemption-plus-byte scanner must observe every name after non-UTF-8 content. | FR-006-AC-7, TC-029 | correct-requirement-no-evidence |
