...
scope: "tests/shared_assurance.rs and assurance/change-assurance.json at 6f13a58"
review_set: subset
relationships:
...
- The scanner extracts inline and literal-block YAML `run:` scripts; YAML
...
- Npm `install` and `i` are found after global options and their values. The
...
| FND-4404 | low | No remaining code defect found. The control is internal Rust assurance infrastructure and does not add a user-authored TL or Quire language surface. | NFR-003, TC-039, `tests/shared_assurance.rs` |
