...
exact upstream repins, nonzero-lower-bound temporal properties, reachable graph
compaction, output-relevant traces, catalog-derived step revisions, and direct
cycle/error regressions. No unresolved code defect or uncovered blocking
...
| FND-206 | medium | Resolved: each pass interns identical kind/span nodes, prunes unreachable nodes, enforces the node ceiling on the compacted graph, and directly tests repeated-state non-convergence plus nondegenerate limits. | FR-002 |
...
| FND-209 | medium | Resolved: evidence behavior rejects skipped or failed governance checks, parameter identity includes semantic implementation files, and checksummed retained evidence is part of the complete gate. | FR-005, NFR-002 |
