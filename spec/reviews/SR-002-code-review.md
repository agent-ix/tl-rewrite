...
| FND-212 | high | Resolved: summary rederivation is unconditional, the one pre-summary legacy record is explicitly migrated as inconclusive, added status files enter the outcome census, and the outer checksum manifest enforces exact file membership within each retained record. | FR-005, NFR-002 |
