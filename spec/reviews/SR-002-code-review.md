...
| FND-210 | low | Resolved: each apply arm now carries its catalog revision with its static rule identity, so the engine has no unreachable catalog-lookup failure or unowned `invalid_catalog` wire disposition; the enabled-rule population proves all 38 maintained identities resolve and execute. | FR-001, FR-002 |
| FND-211 | medium | Resolved: retained trace provenance uses a single budgeted reachability walk over per-source retained operands instead of transitive `BTreeSet` unions. A 100,000-node rewriting Not-chain completed in 313 ms in the local release probe (previous review: 175.34 s), and TC-021 proves the traversal consumes the declared work budget. | FR-002, NFR-001 |
