...
| FND-1008 | medium | FIXED by `agent-ix/tl-rewrite#15`, and **discovered by this pass rather than introduced by it**. `a_control_naming_a_scenario_that_does_not_exist_is_refused` (TC-027) failed intermittently because the probe symlinked `target` into its own scratch directory, so `STORE` resolved back through `target/dangling-probe/target` to the real `target/assurance-store` and the probe shared its Quoin store rather than owning one. Quoin then wrote a non-JSON warning to stdout when its working directory had gone, and `receipt()` parsed stdout as JSON. The corrected baseline is **2/12 failures (17%) without this review's census test and 6/10 (60%) with it**; the census amplified but did not cause the defect, and the with-census population is the one `make ci` actually runs. The fix gives the scratch an ordinary `target/`, shares only the already-produced `target/assurance` inputs, and asserts that its new `target/assurance-store` canonicalizes somewhere different from the real store. Post-fix, the exact probe passed 10/10 sequential repeats and the complete shared-assurance suite with the census included passed 10/10 sequential repeats. Introduced with the probe in issue #9, not here. | NFR-003-AC-3 |
