...
Residual gaps, each DEFERRED to a linked issue, ACCEPTED with a rationale, or
FIXED on a named follow-up revision. The implementation findings this pass
raised and fixed are in `SR-009`.
...
| FND-1008 | medium | FIXED **by merged PR `agent-ix/tl-rewrite#17`**, and **discovered by this pass rather than introduced by it**. `a_control_naming_a_scenario_that_does_not_exist_is_refused` (TC-027) failed intermittently while its scratch symlinked the repository `target`, so the probe's `STORE` resolved to the real `target/assurance-store` rather than a store it owned. Sharing that parent is the coupling proved by the paths; the exact trigger for the observed non-JSON Quoin warning is not established, and the driver deletes only its own unique `run-*` directory. The earlier statement that the failure rate was unchanged with the census filtered out is **withdrawn**: two separate pre-fix campaigns observed **2/12 failures (17%)** from `for i in $(seq 1 12); do cargo test --test shared_assurance -- --test-threads=1 --skip no_local_evidence_framework_remains; done` and **6/10 failures (60%)** from the same loop without `--skip`. Those small samples report two observations; they do not establish that the census amplified the failure. PR #17 creates a real scratch `target/`, shares only `target/assurance`, canonicalizes the repository `target` without requiring the real store to pre-exist, and runs the unmutated driver in the same scratch as a positive control. At reviewed head `ec6668bc869aff853959c32dea743e21f87d7c1c`, `for i in $(seq 1 10); do cargo test --test shared_assurance a_control_naming_a_scenario_that_does_not_exist_is_refused -- --exact --test-threads=1; done` passed 10/10 and `for i in $(seq 1 10); do cargo test --test shared_assurance -- --test-threads=1; done` passed 10/10; exact-head local `make ci CARGO_TARGET_DIR=target/cargo-review` also passed. The isolation construction was byte-identical between that measured head and reviewed landing head `ee5a32956f7425c0c9ed60cdbc06f3bbda26d129`; the latter added two fail-closed branches on paths the passing campaigns did not take, moved the ownership check ahead of `create_dir_all`, and replaced the store-containment predicate with a strictly stronger one. The construction whose isolation the campaigns measured is unchanged. The construction was introduced with issue #9, not by the deletion. | NFR-003-AC-3 |
