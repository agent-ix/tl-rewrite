...
/// TC-074, FR-021-AC-2: serialized refusals never replay as rule evidence.
#[test]
fn tc_074_non_success_statuses_cannot_be_promoted_by_report_replay() {
    use tl_rewrite::BudgetKind;

    let input = fixture("temporal.future.true");
    let successful = run(&input, None);
    assert!(successful.succeeded());
    let cases = [
        (RewriteStatus::UnsupportedProfile, None),
        (
            RewriteStatus::BudgetExhausted,
            Some(InfiniteRewriteFailure::Budget(BudgetKind::WorkUnits)),
        ),
        (
            RewriteStatus::Failed,
            Some(InfiniteRewriteFailure::Internal),
        ),
    ];

    for (status, failure) in cases {
        let mut refused = successful.clone();
        refused.status = status;
        refused.failure = failure;
        refused.steps.clear();
        refused.output_identity = None;
        refused.output_fairness_identity = None;
        refused.output = None;
        refused.output_fairness = None;
        ...
        assert!(!parsed.succeeded(), "{status:?}");
        assert!(
            !replay_infinite(&input, None, &parsed, "test-source"),
            "{status:?}"
        );
    }
}
